...
do
	...
	do
		...
		paste /Users/robertreynolds/research/bayes-morph-soc-net/artificial-data/id_column_24.txt <(paste <(printf "typeFreq,$(grep lexeme_type_freq_list ${each} | cut -d [ -f 2 | sed 's/]//g' | sed 's/ //g')" | tr , \\n) "/Users/robertreynolds/research/bayes-morph-soc-net/artificial-data/data_${input}.txt" | cut -f 1,3-) > ${table}
	done
done
