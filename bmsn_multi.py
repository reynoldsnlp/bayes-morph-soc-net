...
from mesa_augmenter import MultiScheduler
...
def mlt_prob(p_dict, d_dict):  # deprecated in favor of mlt_prob_log
...
def metalog(model, explicit_filename=None):
    ...
    # adict['morph_filename'] = adict['morph_filename'].split('/')[-1].lstrip('data_').rstrip('.txt')  # noqa: E501
...
class MorphAgent(mesa.Agent):
    ...
    def step(self):
        ...
        if self.is_adult and not self.RIP:  # 'active' adults
            ...
            for a in self.connections:
                with open('tmp/' + self.model.out_filename + '_' + str(a) + '.tmp') as input_file:  # noqa: E501
                    for line in input_file:
                        self.input.append(tuple(line.rstrip().split('\t')))
    ...
    def process_input(self):
        """Process input using Bayesian learning and write output file."""
        # data distribution
    ...
    def speak(self):
        ...
        speech_fname = 'tmp/' + self.model.out_filename + '_' + str(self.unique_id) + '.tmp'  # noqa: E501
        with open(speech_fname, 'w') as out_file:
            for d in out:
                print(*d, sep='\t', file=out_file)
