...
As described in:
    ...
    Ackerman, Farrell, James P. Blevins, & Robert Malouf. 2009. "Parts
        and wholes: Patterns of relatedness in complex morphological
        systems and why they matter." In Analogy in Grammar: Form and
        Acquisition, ed. by James P. Blevins & Juliette Blevins, 54-82.
        Oxford: Oxford University Press.

    Ackerman, Farrell and Robert Malouf. 2013. "Morphological
        organization: The Low Conditional Entropy Conjecture." Language
        89:429-464.
...
def cond_entropy(paradigm):
    """Calculate pairwise conditional entropy in an inflectional paradigm."""
    ...
    for r in paradigm.columns:
        ...
        for c in paradigm.columns:
            if r != c:
                v = (paradigm[r] +
                     ':' +
                     paradigm[c]).value_counts(normalize=True)
                result[c][r] = -sum(v * np.log2(v)) - H
    return result


def bootstrap(paradigm, R=99):
    """Estimate the average conditional entropy expected in a given paradigm.

    (absent implicational relations)
    """
...
if __name__ == '__main__':
    ...
    print('Avg. cond. entropy = %.3f bits' %
          (cond_entropy(paradigm).mean().mean()))
