...
## updated by Jeff for complexity paper -- 10/2016
...
doCalc.fnc = function(file, weighted){
	...
	input_folder<-paste0(getwd(),"/1_matrices")
	forms<-read.delim(file, header=T, sep="\t", stringsAsFactors=FALSE)
	
	colNames_temp<-as.character(names(forms))
	#creating an index for the first MPS (which is NomSg so I can ignore everything before it)
	index_NomSg <- grep("MPS_A", colNames_temp)
	index_typeFreq <- grep("typeFreq", colNames_temp)
	
	##removing first column (class index) so input file is correctly formatted
	forms<-forms[,c(index_typeFreq,index_NomSg:ncol(forms))]
...
#							typeFreqTable<-rbind(typeFreqTable, row)
						}	
					}
				}
				cat(".")
